...
def details(request, currId):
    ...
    serialized_data = scholarshipSerializer(user_details, context={'request': request})
...
def update(request, currId):
    ...
    serializer = scholarshipSerializer(user_details,data=request.data, context={'request': request})
    response_sent={}
...
def list(request):
    try:
        ...
        return Response(status=status.HTTP_404_NOT_FOUND)
    serialized_data = scholarshipSerializer(user_details, many=True, context={'request': request})
    return Response(serialized_data.data)
